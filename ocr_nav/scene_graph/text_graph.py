...
@dataclass
class TextBag:
    """Class for storing text observations and associated 3D locations.
    One text label may be observed as different variants due to OCR errors. For example,
    in one frame "book" was observed but in another frame, the same label was
    recognized as "b00k". We group them into one TextBag node in the pose graph.
    All variants are stored as keys in the text_dict attribute. The values are
    the list of frame ids where the text was observed.

    Attributes:
    text_dict: map from text to list of frame ids where the text was observed.
