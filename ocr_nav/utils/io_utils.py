...
def search_latest_poses_within_timestamp_range(
    ...
    Returns:
        ...
        Tuple containing the latest pose and its timestamp, or (None, None) if not found.
    """
...
def numpy_img2bytes_pil(image: np.ndarray, image_format: str = "jpeg") -> bytes:
    """Convert an RGB image's NumPy array into bytes for LLM to consume

    Args:
        image (np.ndarray): The RGB image as a NumPy array.
        image_format (str, optional): The format to encode the image in. Defaults to "jpeg".
...
def bytes_to_base64_string(image_bytes: bytes) -> str:
    import base64
...
class SubscriberIO(Node):
    def __init__(
        self,
        camera_info_topic: str = "/zed/zed_node/left/color/rect/image/camera_info",
        segmentation_mask_topic: str = "/segmentation_mask",
        anchor_lidar_id: int = 0,
        lidar_topic_list: list[str] = ["/glim_ros/points_corrected", "/rslidar_points"],
        lidar_frame_ids: list[str] = ["livox_mid360_imu", "robosense_e1r"],
        camera_frame_id: str = "zed_left_camera_optical_frame",
        world_frame_id: str = "odom_glim",
        accumulated_anchor_lidar_topic: str = "/acc_anchor_lidar",
        tf_buffer_sec: int = 600,
    ):
        """An ROS2 Node that subscribes to camera info, segmentation mask, and multiple LiDAR topics.
        It synchronizes the incoming messages and provides methods to access the latest synchronized data,
        camera intrinsics, image size, and LiDAR-to-camera transforms.

        Args:
            camera_info_topic (str, optional): _description_. Defaults to
                "/zed/zed_node/left/color/rect/image/camera_info".
            segmentation_mask_topic (str, optional): _description_. Defaults to "/segmentation_mask".
            anchor_lidar_id (int, optional): _description_. Defaults to 0.
            lidar_topic_list (list[str], optional): _description_. Defaults to
                ["/glim_ros/points_corrected", "/rslidar_points"].
            lidar_frame_ids (list[str], optional): _description_. Defaults to ["livox_mid360_imu", "robosense_e1r"].
            camera_frame_id (str, optional): _description_. Defaults to "zed_left_camera_optical_frame".
            world_frame_id (str, optional): _description_. Defaults to "odom_glim".
            accumulated_anchor_lidar_topic (str, optional): _description_. Defaults to "/acc_anchor_lidar".
            tf_buffer_sec (int, optional): _description_. Defaults to 600.
        """
        ...
        self.lidar_subscriber_list = []
        for lidar_topic in lidar_topic_list:
            sub = Subscriber(self, PointCloud2, lidar_topic)
            self.lidar_subscriber_list.append(sub)
        self.sub_mask = Subscriber(self, ROSImage, segmentation_mask_topic)
        self.pub_acc_anchor_lidar = self.create_publisher(PointCloud2, accumulated_anchor_lidar_topic, 10)

        self.synchronizer = ApproximateTimeSynchronizer(
            [self.sub_mask] + self.lidar_subscriber_list,
            queue_size=1000,
            slop=5,
        )
    ...
    def synchronized_callback(self, mask_msg: ROSImage, *lidar_msgs_list: PointCloud2):
        try:
            # lookup_transform(target_frame, source_frame, time)
            listened_time = rclpy.time.Time.from_msg(lidar_msgs_list[self.anchor_lidar_id].header.stamp)
            tf_anchor_lidar_to_world = self.lookup_tfs(
                self.world_frame_id, self.lidar_frame_ids[self.anchor_lidar_id], listened_time
            )  # (4, 4)
            lidar_pc_list = []
            for lidar_msg in lidar_msgs_list:
                lidar_pc_np = convert_pc2_to_numpy(lidar_msg)  # (N, 3)
                lidar_pc_list.append(lidar_pc_np)
            mask_np = convert_image_to_numpy(mask_msg)  # (H, W, C) or (H, W)

            self.get_logger().info(f"Found TF at: \n{tf_anchor_lidar_to_world}")
            self.sync_buffer.append((lidar_pc_list, mask_np, tf_anchor_lidar_to_world))

        except TransformException as ex:
            self.get_logger().info(f"Could not transform /odom_glim to /livox_mid360_imu: {ex}")

    def get_latest_sync_data(
        self,
    ) -> tuple[list[np.ndarray], np.ndarray, np.ndarray] | None:
        """Get the latest synchronized data containing: a list of LiDAR point clouds, Ground segmentation
        mask, and a tf (4,4) that transforms point cloud from the anchor LiDAR frame to the Global frame

        Returns:
            Union[Tuple[list[np.ndarray], np.ndarray, np.ndarray], None]:
            (lidar_pc_list list[(M, 3)], mask_np (H, W), tf_livox_to_odom (4, 4))
        """
    ...
    def get_lidar2camera_tfs(self) -> np.ndarray:
        try:
            self.lidar2camera_tfs = []
            for lidar_frame_id in self.lidar_frame_ids:
                tf = self.lookup_tfs(self.camera_frame_id, lidar_frame_id, rclpy.time.Time())
