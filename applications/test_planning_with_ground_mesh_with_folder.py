...
def main():
    ...
    nodes = list(ground_mesh.floor_graph.nodes(data=True))
    np.random.seed(1)
    added_mesh_actor = []
    colors = ["skyblue", "firebrick", "grey", "salmon", "forest_green"]
    for test_i in range(5):
        start_node = nodes[np.random.randint(0, len(nodes))][0]
        goal_node = nodes[np.random.randint(0, len(nodes))][0]
        start_pos = np.array(ground_mesh.floor_graph.nodes[start_node]["pos"])
        goal_pos = np.array(ground_mesh.floor_graph.nodes[goal_node]["pos"])
        print(f"Planning from {start_pos} to {goal_pos}...")
        path_pos = ground_mesh.plan_global_path(start_pos, goal_pos)

        # plot the starting node and the goal node
        start_cube = draw_cube(start_pos, size=0.5, color="blue")
        start_actor = plotter.add_mesh(start_cube, color="blue", name="start_cube")
        added_mesh_actor.append(start_actor)
        goal_cube = draw_cube(goal_pos, size=0.5, color="red")
        goal_actor = plotter.add_mesh(goal_cube, color="red", name="goal_cube")
        added_mesh_actor.append(goal_actor)
        ...
        for i in pbar:
            ...
            plotter.add_mesh(line, line_width=10, color=colors[test_i], render_lines_as_tubes=True)
            plotter.update()
        plotter.update()
    plotter.show()
