...
import cv2
import rclpy
from ocr_nav.utils.io_utils import FolderIO, SubscriberIO
...
def main():
    ...
    executor.add_node(subscriber_io)
    thread = threading.Thread(target=executor.spin, daemon=True)
    thread.start()

    ground_mesh = FloorGraph(voxel_size=0.2)
    ground_mesh.build_floor_graph_with_rosio(subscriber_io, vis=False)
