...
def main():
    ...
    floor_graph = FloorGraph(voxel_size=0.2)
    floor_graph.build_floor_graph_with_bagio(bagio, segmenter, vis=False)
    stairs_pc_full = o3d.geometry.PointCloud()
    for stair_pc in floor_graph.stairs_pc_list:
    ...
    ground_point_down = downsample_point_cloud(floor_graph.full_ground_pc, voxel_size=0.2)
    ...
    total_pose_nodes = sorted(
        [node for node, att in floor_graph.pose_graph.G.nodes(data=True) if isinstance(node, int)]
    )
    ...
    for node, att in floor_graph.pose_graph.G.nodes(data=True):
    ...
    for edge in floor_graph.pose_graph.G.edges():
        node1, node2 = edge
        if isinstance(node1, int) and isinstance(node2, int):
            pose1 = floor_graph.pose_graph.G.nodes[node1]["pose"]
            pose2 = floor_graph.pose_graph.G.nodes[node2]["pose"]
    ...
    plotter.show()
    floor_graph.save_floor_graph((args.output_dir / f"{args.output_graph_name}.json").as_posix())
    floor_graph.load_floor_graph((args.output_dir / f"{args.output_graph_name}.json").as_posix())
