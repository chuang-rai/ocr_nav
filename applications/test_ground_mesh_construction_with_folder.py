...
def main():
    ...
    ground_mesh = GroundMesh(voxel_size=0.1)
    ground_mesh.build_ground_mesh_with_folder(folderio, filter_obs=True, debug=False, vis=False)
    ...
    ground_point_down = downsample_point_cloud(ground_mesh.ground_pc_rs, voxel_size=0.2)
    ...
    total_pose_nodes = sorted([node for node, att in ground_mesh.textmap.G.nodes(data=True) if isinstance(node, int)])
    ...
    for node, att in ground_mesh.textmap.G.nodes(data=True):
    ...
    for edge in ground_mesh.textmap.G.edges():
        node1, node2 = edge
        if isinstance(node1, int) and isinstance(node2, int):
            pose1 = ground_mesh.textmap.G.nodes[node1]["pose"]
            pose2 = ground_mesh.textmap.G.nodes[node2]["pose"]
    ...
    plotter.show()
    GroundMesh.save_voronoi_graph(ground_mesh.fused_graph, root_path / "ground_fused_graph.json")
